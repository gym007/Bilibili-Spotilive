...
async def main():
    ...
    import obs_widget
    obs_widget.set_spotify_controller(spotify_ctrl)  # 设置 SpotifyController 实例到
    ...
    print(f"[{room_id}]{timestamp()}[INFO] ──────────────────────────────")
    print(f"[{room_id}]{timestamp()}[INFO] OBS 小组件已启动，请在 OBS 添加浏览器采集：")
    print(f"[{room_id}]{timestamp()}[INFO] ▶️  Spotify Now Playing 小组件：")
    print(f"[{room_id}]{timestamp()}[INFO]     👉 http://localhost:5001/nowplaying_widget/")
    print(f"[{room_id}]{timestamp()}[INFO] ▶️  点歌机状态小组件：")
    print(f"[{room_id}]{timestamp()}[INFO]     👉 http://localhost:5001/queue_widget/")
    print(f"[{room_id}]{timestamp()}[INFO] ──────────────────────────────\n")
