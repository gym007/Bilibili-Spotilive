...
from apis.obs_widget import (
    create_message_message,
    create_queue_message
)
from apis.api_server import emit_message, emit_queue, clear_queue
...
def set_player_spotify_controller(controller):
    """
    设置 Spotify 控制器
    :param controller: SpotifyController 实例
    """
    global spotify_ctrl
...
async def _emit_queue_status():
    """根据队列状态更新 widget：有队列则展示，否则显示点歌引导"""
    emit_queue = await create_queue_message()
    if emit_queue:
        await emit_queue_to_widget(emit_queue)
    else:
...
async def get_next_item() -> Optional[Dict]:
    """
    按优先级从三条队列中取下一首 song_item
    优先级: streamer -> guard -> normal
    """
...
async def player_loop(room_id: str):
    """
    后台循环：检测 Spotify 播放状态、处理点歌与默认播放流程
    """
    ...
    while True:
        try:
            ...
            if not is_playing or current_request is None:
                next_item = await get_next_item()
                if next_item:
                    current_request = next_item
                    song = next_item["song"]
...
async def request_song(request: Dict, request_perm: bool) -> bool:
    ...
    try:
        ...
        if current_request is None:
            current_request = {"song": song, "request": request}
            # 直接播放
            ...
            if user.get("is_streamer"):
                await song_queue_streamer.add_song({"song": song, "request": request})
                btn = "加入主播队列"
            elif user.get("privilege_type", 0) > 0:
                await song_queue_guard.add_song({"song": song, "request": request})
                btn = "加入大航海队列"
            else:
                await song_queue.add_song({"song": song, "request": request})
                btn = "加入普通队列"
